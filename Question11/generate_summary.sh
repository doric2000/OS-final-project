...
for gcov_file in coverage/*.cpp.gcov; do
    if [ -f "$gcov_file" ]; then
        ...
        # Count total lines, executed lines, and missed lines from gcov format
        # Only count actual source lines (not metadata lines starting with -)
        total_executable=$(grep -E "^[[:space:]]*[0-9]+:" "$gcov_file" | wc -l)
        executed_lines=$(grep -E "^[[:space:]]*[0-9]+:" "$gcov_file" | grep -v "^[[:space:]]*0:" | wc -l)
